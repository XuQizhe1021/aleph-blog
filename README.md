...
- 首页启动层：进入首页时随机全屏展示 `public/start` 图片，支持左右轮换、上滑收起、顶栏再次拉下
- 本地管理后台：上传 md、同名覆盖更新、删除、检索、预览（含 LaTeX）、上传打赏二维码图片、管理首页启动图
...
- 上传/删除文章，修改文章分类，预览渲染结果
- 上传打赏图（会覆盖 `public/reward.*`）
- 上传首页启动图（保存到 `public/start`）
